...
class Booking < ActiveRecord::Base
  ...
  def overlaps
    overlapping_bookings = [ 
      appointment.bookings.end_during(start_time, end_time),
      appointment.bookings.start_during(start_time, end_time),
      appointment.bookings.happening_during(start_time, end_time),
      appointment.bookings.enveloping(start_time, end_time),
      appointment.bookings.identical(start_time, end_time)
    ].flatten
  ...
  def start_date_cannot_be_in_the_past
    if start_time && start_time < DateTime.now + (15.minutes)
      errors.add(:start_time, 'You are unable to request this date and time')
    end
  end
